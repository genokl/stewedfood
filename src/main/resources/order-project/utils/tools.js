class tools {
  ...
  getIsWhite() {
    console.log('jjjj');
    var time = new Date();
    var hours = time.getHours(); //获取时间的小时搜索
    if (hours <= 7 || hours >= 21) {
      console.log('夜间');
      return false;
    } else {
  ...
  alert(title, content, callbackFn){
    wx.showModal({
      title: title,
      content: content,
 ...
 */
  ...
  var currentdate="";
  if (this.getDate(date) == this.getDate(new Date())) {
      currentdate = "今天 " +date.getHours() + seperator2 + date.getMinutes() 
